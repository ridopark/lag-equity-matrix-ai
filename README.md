...
**[Synthetic Signal Cascade](https://claude.ai/code/artifact/044ed8f9-e475-431c-883c-b2701f3cb5b0)**
— the graph executing over six fabricated signals, replayed at its measured
timings. Six invented tickers over an invented 166-symbol universe, each
candidate constructed to land on a different verdict branch, so all four
outcomes are visible in one run.

The trace is real; the market is not. Nothing on that page is a price, a ticker
or a position that exists. It is generated by
`scripts/capture_trace.py --synthetic` from the same committed universe the CI
guard uses, and checked in at `docs/trace-synthetic.json`.

One detail worth watching: `graph_retriever` fires six times but `leader_state`
