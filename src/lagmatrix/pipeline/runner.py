...
from lagmatrix.adapters.llm import build_analyst_client
...
def _load_bars(start: date, end: date, path: str = BARS_PATH) -> pd.DataFrame | None:
    """The trailing window of `data/bars.parquet`, for `day_trade_perspective`.

    Generated by scripts/fetch_bars.py; a fresh clone won't have it, so a
    missing file just means no bars data rather than a hard failure (same
    idiom as `_load_excluded_symbols`).
    """
    try:
    ...
    start_ts, end_ts = pd.Timestamp(start, tz=tz), pd.Timestamp(end, tz=tz)
